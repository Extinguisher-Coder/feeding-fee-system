...
import "./CashierWeeklyReportPage.css";

const CashierWeeklyReportPage = () => {
  ...
  const fetchSummary = async () => {
    try {
      ...
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_API_URL}/cashier-summary/weekly?termName=${termName}`
      );
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Cashier Weekly Summary");
    XLSX.writeFile(wb, `CashierWeeklySummary_${termName}.xlsx`);
  };
  ...
  const printReport = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Cashier Weekly Summary</title>
          <style>
        ...
        <body>
          <div class="logo-header">
            ...
            <h2>Feeding Fees Management System</h2>
  ...
  return (
    <div className="cashier-weekly-report">
      <h2>Cashier Weekly Summary</h2>
                  ...
                  <tbody>
                    {Object.entries(cashierData.weeks).map(([weekLabel, stats], index) => (
                      <tr key={index}>
                        ...
                        <td>
                          <span
                            className={`status-badge ${stats.status
                              .toLowerCase()
                              .replace(" ", "-")}`}
                          >
                            {stats.status}
...
export default CashierWeeklyReportPage;
