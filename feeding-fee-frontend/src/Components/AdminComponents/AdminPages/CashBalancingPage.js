...
import './CashBalancingPage.css';
import * as XLSX from 'xlsx';
import { useNavigate } from 'react-router-dom'; // 👈 Import this

const CashBalancingPage = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_API_URL}/cash-reconciliation/summary`);
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Cash Balancing Report');
    XLSX.writeFile(workbook, 'Cash_Balancing_Report.xlsx');
  };

  return (
    <div className="cash-page">
      <h2 className="pagetitle">Cash Balancing</h2>
    ...
    const routeMap = {
      Admin: '/admin/add-received',
      Accountant: '/accountant/add-received',
    };
    ...
    const { role } = JSON.parse(localStorage.getItem('user'));
    const routeMap = {
      Admin: '/admin/daily-balancing',
      Accountant: '/accountant/daily-balancing',
    };
    navigate(routeMap[role] || '/unauthorized');
  }}>
...
export default CashBalancingPage;
