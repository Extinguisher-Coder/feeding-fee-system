...
import './AddCashReceivedForm.css';

const AddCashReceivedForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_API_URL}/cash-reconciliation/record`, {
        method: 'POST',
      ...
      const result = await response.json();
      if (response.ok) {
        alert('Amount recorded successfully.'); // ✅ Show alert
...
export default AddCashReceivedForm;
