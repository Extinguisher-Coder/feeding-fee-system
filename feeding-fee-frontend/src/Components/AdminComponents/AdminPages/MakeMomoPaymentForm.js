...
import './MakeMomoPaymentForm.css';  // create this for styling like MakePaymentForm.css
...
const MakeMomoPaymentForm = ({ student, cashier, onClose }) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      // Adjust this API endpoint to your backend route handling MoMo payments
      await axios.post(`${API_BASE_URL}/payments/make/${form.studentId}`, {
        ...form,
        amount: parseFloat(form.amount),
      });
      alert('✅ MoMo Payment saved successfully!');
      onClose();
    } catch (err) {
      console.error(err);
      alert('❌ Failed to save MoMo payment.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="momo-payment-form-container">
      <form className="momo-payment-form" onSubmit={handleSubmit}>
        ...
        {!currentTerm && (
          <p style={{ color: 'red', marginTop: '10px' }}>
            ⚠️ A current term must be set before MoMo payments can be accepted.
          </p>
        )}
