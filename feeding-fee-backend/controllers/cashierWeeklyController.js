...
// Utility: Get weekly ranges labeled as week1, week2, ...
const getWeeklyRanges = (startDate, endDate) => {
  ...
  while (current <= endDate) {
    ...
    // ✅ Ensure full day is included (up to 23:59:59.999)
...
// @desc Get weekly summary for all cashiers by term
// @route GET /api/cashier-summary/weekly?termName=Term%203%20-%202025
exports.cashierWeeklySummary = async (req, res) => {
  try {
    ...
    const summary = await Promise.all(
      cashiers.map(async (cashier) => {
        ...
        for (const week of weeklyRanges) {
          ...
          // Total collected by cashier during the week
          const recordedAgg = await History.aggregate([
            {
              $match: {
                cashier,
          ...
          // Total handed over to accountant by cashier during the week
          const accountedAgg = await BalanceHistory.aggregate([
            {
              $match: {
                cashier,
    ...
    console.error('Cashier weekly summary error:', err);
