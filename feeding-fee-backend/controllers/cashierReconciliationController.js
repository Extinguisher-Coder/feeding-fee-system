// cashierReconciliationController.js

const Term = require('../models/termModel');
const historyModel = require('../models/historyModel');

exports.cashierReconciliation = async (req, res) => {
  try {
    ...
    // Process weekly payments with cashier breakdown
    const results = await Promise.all(
      weeks.map(async ({ weekLabel, weekStart, weekEnd }) => {
        ...
        for (const payment of payments) {
          const cashier = payment.cashier || 'Unknown';
          if (!cashierSummary[cashier]) {
            cashierSummary[cashier] = 0;
          }
          cashierSummary[cashier] += payment.amountPaid || 0;
        }
    ...
    console.error('Error in cashier reconciliation:', error);
    res.status(500).json({ error: 'Server error while generating cashier reconciliation.' });
  }
};
